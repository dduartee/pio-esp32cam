...
// #define BLUETOOTH "ESP32BT"
  ...
  DumbDisplay dumbdisplay(new DDWiFiServerIO("SPIKE", "casa526610"));
#endif
...
const tflite::Model* model = ::tflite::GetModel(mnist_quantized_micro_tflite);
...
void setup() {
  ...
  // Apenas ops necessárias para Dense(16)->Dense(10) com softmax
  tflite::MicroMutableOpResolver<2>* micro_op_resolver = new tflite::MicroMutableOpResolver<2>();
  // Camadas densas e softmax do modelo MNIST
...
void loop() {
  ...
  if (detectImageLayer->getFeedback() != NULL) {
    statusLayer->clear();
    statusLayer->pixelColor(DD_COLOR_red);
    ...
    // process inference results for MNIST digit prediction
    TfLiteTensor* output = interpreter->output(0);
    // encontrar o dígito com maior probabilidade
    int predicted = 0;
    ...
    // exibir resultado no dumbdisplay
    dumbdisplay.writeComment(String("Predicted digit: ") + String(predicted));
    statusLayer->clear();
    statusLayer->pixelColor(DD_COLOR_blue);
    statusLayer->writeCenteredLine(String("Dígito previsto: ") + String(predicted), 1);
    dumbdisplay.writeComment(String("Dígito previsto: ") + String(predicted));
